...
from app.models import Product, Order, Review, LikeProduct, LikeReview, Profile, Category, Orders_products
...
def index(request):
    ...
    return render(request, "main-page.html", {'search_form': search_form, 'products': current_page, 'pages': pages, 'top_categories': top_categories, 'top_products': top_products})
...
def products(request, sort_name):
    ...
    current_page, pages = my_paginator(request, products, 10)
    return render(request, "products.html", {'search_form': search_form, 'sort_name': sort_name, 'products': current_page, 'pages': pages, 'top_categories': top_categories, 'top_products': top_products})
...
def product(request, number):
    ...
    product = Product.objects.get_id(number)
    reviews = Review.objects.by_product(number)
    current_page, pages = my_paginator(request, reviews, 5)
    return render(request, "single-product.html", {'search_form': search_form, 'product': product, 'reviews': current_page, 'pages': pages, 'top_categories': top_categories, 'top_products': top_products, 'form': form})


def category(request, category_name):
    products = Product.objects.get_category(category_name)
    current_page, pages = my_paginator(request, products, 10)
    return render(request, "category-sort.html", {'search_form': search_form, 'products': current_page, 'pages': pages, 'top_categories': top_categories, 'top_products': top_products, 'category': category_name})
...
def login(request):
    ...
    return render(request, 'login.html', {'search_form': search_form, 'top_categories': top_categories, 'top_products': top_products, 'form': form})
...
def signup(request):
            ...
            try:
                ...
                try:
                    ...
                    Profile.objects.create(
                        user=user, birth_date=form_birth_date, sex=form_sex, address=form_address, avatar=form_avatar)
                    return redirect("index")
                else:
    ...
    return render(request, 'register.html', {'search_form': search_form, 'top_categories': top_categories, 'top_products': top_products, 'form': form})
...
def settings(request):
    if request.method == "GET":
        profile = Profile.objects.get(user=request.user)
        data = {"email": profile.user.email, "first_name": profile.user.first_name, "last_name": profile.user.last_name, "address": profile.address}
        ...
        if form.is_valid():
            ...
            return redirect("index") 
    return render(request, "settings.html", {'search_form': search_form, 'top_categories': top_categories, 'top_products': top_products, 'form': form})
...
def bag(request):
    ...
    for x in cart.cart:
        ...
        _product = {'id': product.id, 'title': product.title, 'cost': str(product.cost), 'count': cart.cart[x]['cnt'], 'total': total}
        products.append(_product)

    total_price = int(cart.get_total_cost())

    return render(request, "bag.html", {'search_form': search_form, 'products': products, 'total_price': total_price, 'top_categories': top_categories, 'top_products': top_products})
...
def search(request):
    ...
    return render(request, "products.html", {'search_form': _search_form, 'sort_name': sort_name, 'products': current_page, 'pages': pages, 'top_categories': top_categories, 'top_products': top_products})
...
def grade(request):
    ...
    if data['type'] == 'review':
        ...
        if (len(review_likes)):
            ...
            like = LikeReview.objects.create(mark=inc, profile=request.user.profile, review=review)
        ...
        if (len(product_likes)):
            ...
            like = LikeProduct.objects.create(mark=inc, profile=request.user.profile, product=product)
            response.set_cookie(key='new', value='True')
...
def order(request, number):
    ...
    for product in order.products.all():
        cnt = Orders_products.objects.filter(product_id=product.id, order_id=order.id)[0].cnt
        total = cnt * int(product.cost)
        _product = {'id': product.id, 'title': product.title, 'cost': product.cost, 'count': cnt, 'total': total}
        products.append(_product)
    ...
    return render(request, "order.html", {'search_form': search_form, 'order': order, 'total_price': result[0][0], 'products': products, 'top_categories': top_categories, 'top_products': top_products})
...
def order_history(request):
    ...
    with connection.cursor() as cursor:
        for order in orders:
            ...
            _order = {'id': order.id, 'order_date': str(order.order_date.strftime("%m.%d.%Y, %H:%M")), 'status': order.status, 'comment': order.comment, 'total_price': result[0][0]}
            _orders.append(_order)

    return render(request, "order_history.html", {'search_form': search_form, 'orders': _orders, 'top_categories': top_categories, 'top_products': top_products})
...
def make_order(request):
    ...
    for product_id in cart.cart:
        ...
        if product.count == 0:
            ...
            comment += "Product quantity changed to %s: %s" % (cart.cart[product_id]['cnt'], product.title)
    ...
    if len(cart) > 0:
        ...
        for product_id in cart.cart:
            product = Product.objects.get(id=product_id)
            order.products.add(product, through_defaults={"cnt": int(cart.cart[product_id]['cnt'])})
